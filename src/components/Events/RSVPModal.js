// File: src/components/events/RSVPModal.js
...
// Utility function to handle RSVP responses
export function handleRSVPResponse(response) {
  const { status, token, eventId } = response;
  
  if (status === 'confirmed') {
    // Store temporary token in localStorage with expiration
